...
# Definição dos parâmetros SARIMA para 'sales' ---------------------------------
...
fit.sarima.sales <- Arima(df_semana.sales.ts, order = c(p, d, q), seasonal = list(order = c(P, D, Q), period = 13))
summary(fit.sarima.sales)
checkresiduals(fit.sarima.sales)

# Definição dos parâmetros SARIMA para 'revenue' ---------------------------------
...
fit.sarima.revenue <- Arima(df_semana.revenue.ts, order = c(p, d, q), seasonal = list(order = c(P, D, Q), period = 13))
summary(fit.sarima.revenue)
checkresiduals(fit.sarima.revenue)
...
data.frame(AIC = round(c(fit.sarima.sales$aic, fit.sarima.revenue$aic), 1),
           BIC = round(c(fit.sarima.sales$bic, fit.sarima.revenue$bic), 1),
           row.names = c("sales", "revenue"))

# Previsões dos modelos para 5 semanas -----------------------------------------

# Previsão 'sales'
predict.sarima.sales <- forecast(fit.sarima.sales, h = 5)
predict.sarima.sales
...
# Previsão 'revenue'
predict.sarima.revenue <- forecast(fit.sarima.revenue, h = 5)
predict.sarima.revenue
plot(predict.sarima.revenue,
     sub = "manual setting of parameters",
     col = "navyblue",
     lwd = 2)
